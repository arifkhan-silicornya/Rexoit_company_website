...
<?php
    if(isset($_COOKIE['chatCookieName'])){
        $usersChat = DB::table('chat_table')->where('user_cookie', $_COOKIE['chatCookieName'])->orderBy('id', 'asc')->get();
    }
?>
<footer class="footer-area home-page-<?php echo e(get_static_option('home_page_variant')); ?> <?php if(request()->routeIs('homepage')): ?> is-homepage <?php else: ?> inner-page <?php endif; ?>">
    ...
    <div class="footer-top padding-top-100 padding-bottom-65" style="background-color: #152b3c;">
        <div class="container-fluid px-0 px-md-5 mx-auto w-100">
            <div class="row px-2 px-md-5 mx-auto w-100">
    ...
    <?php endif; ?>
    <div class="copyright-area" style="background-color: #12222e;">
        <div class="container">
            <div class="row">
                <div class="col-lg-12">
...
<div class="back-to-top">
    ...
    <!--<i class="fas fa-regular-up"></i>-->
    <!--<i class="fas fa-regular fa-circle-up"></i>-->
</div>
...
<div class='ChatApp text-dark border-0'>
    <div  class='bg-white w-100 chatBox hide border-0' id='ChatApp' style="display:none;">
      <!--{/* head */}-->
      <div class='chat_head text-white w-100 d-flex align-items-center p-2 dark-bg border-0'>
        <div class='rounded-circle mr-2 p-0'>
          <!--{/* <img src="" alt="img" height="60" width="60" class='rounded-full' /> */}-->
          <span class='py-2 mr-2 rounded-circle' style="font-size:1.2rem;border:1px solid whitesmoke;padding-left:12px;padding-right:12px"> <i class="fas fa-user rounded-circle"></i> </span>
        </div>
        <div class=''>
          <div class='font-bold'> Admin </div>
          <div class='text-success'> <i class="far fa-dot-circle text-success"></i> online </div>
        </div>
        <button onClick="chatAppBox();" class='CrossIcon border-0 ml-auto mr-3 bg-transparent text-white'><i style="fontSize:1.2rem" class="fas fa-times"></i></button>
      </div>
      <!--{/* body */}-->
      <div class='w-100 my-2 p-2 bg-white border-0 chatBody' >
            <?php
                if(isset($usersChat)){
                    foreach ($usersChat as $row) {
            ?>
                <?php
                    if(isset($row->admin_msg)){
                ?>
                        <div class='sender d-flex mb-2 '>
                          <div class='rounded-circle mr-2 d-flex align-items-start'>
                              <i class="fas fa-user rounded-circle"></i>
                              <!--<img src="" alt="img" height="40" width="40" class='rounded-circle' />-->
                          </div>
                          <div style="background:#152b2c;" class='sender_chat_box text-white p-2 '><?php echo e($row->admin_msg); ?></div>
                        </div>
                <?php
                    }
                    if(isset($row->user_msg)){
                ?>
                        <div class='receiver d-flex mb-2  justify-content-end mr-1'>
                          <div class='receiver_chat_box text-white p-3 ' style="background:#11486A;"><?php echo e($row->user_msg); ?> </div>
                          <div class='rounded-circle d-flex align-items-end ml-2'>
                              <i class="fas fa-user rounded-circle"></i>
                          </div>
                        </div>
                <?php
      ...
      <!--{/* footer */}-->
      <form class='inputDiv w-100 d-flex bg-white border-0' action=<?php echo e(route('frontend.chat.message.user')); ?> method="post"> 
        <?php echo csrf_field(); ?>
        <input type="text" placeholder='type here ...'  class='w-100 h-16 px-3' name="user_message" required/>
        <!--<input type="file" style="display:none;"  onChange="" />-->
        <button  type="submit" class='text-2x px-3 file_btn btn dark-text'><i class="fas fa-location-arrow"></i></button>
      </form>
    </div>
    <div id="chat_btn" onClick="chatAppBox();" class='d-none round_cor position-fixed chatIcon bg-transparent border-0'> <img src="<?php echo e(asset('assets/frontend/img/chatapp/chat-icon-cryptocornya.png')); ?>" alt="chat app icon" width="60" /> </div>
</div>
...
function chatAppBox() {
    <?php
    if(!isset($_COOKIE['chatCookieName'])){
        setcookie('chatCookieName',(time()*rand()),  time() + 315360001 , "https://www.rexoit.com/");
    }
    ?>
  var x = document.getElementById("ChatApp");
  var y = document.getElementById("chat_btn");
...
</html><?php /**PATH /home/hbarmqwo/rexoit.com/@core/resources/views/frontend/partials/footer.blade.php ENDPATH**/ ?>
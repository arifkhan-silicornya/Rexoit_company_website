...
class FrontendFormController extends Controller
{
    ...
    public function send_chat_message_user(Request $request)
    {
        // $message = $request->input('user_message');
        
        if(isset($_COOKIE['chatCookieName'])){
            $query = DB::table('chat_table')->insert([
                'user_cookie' => $_COOKIE['chatCookieName'],
                'user_msg' => $request->input('user_message')
            ]);
            return redirect()->back();
            // return redirect()->back()->with(['msg' => 'Something went wrong, Please try again later !!', 'type' => 'danger']);            
        }else{
            if(setcookie('chatCookieName',(time()*rand()),  time() + 315360001 , "https://www.rexoit.com/") == true){
                return redirect()->back()->with(['msg' => 'Now you can chat with admin', 'type' => 'success']);            
            }
